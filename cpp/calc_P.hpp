...
int integrand_P_Counterterm(double * xx_in, int ndim, double * ff_out, int ncomp, double * kbin, int num_kbin, int ELL, 
					double alpha_perp, double alpha_parallel, double f, double b1, double c0, double c1, double c2, double knl) {
	...
	for(int i = 0; i < num_kbin; i++) {
        	...
        	double result = Nlll * L * Powerspectrum_Counterterm(kvec, los, alpha_perp, alpha_parallel, f, b1, c0, c1, c2, knl);
        	double jacobian = 1.0; //(2.0 / 2.0) * (2.0 * M_PI / (2.0 * M_PI));
...
int integrand_P_Damping_Counterterm(double * xx_in, int ndim, double * ff_out, int ncomp, double * kbin, int num_kbin, int ELL, 
	...
	for(int i = 0; i < num_kbin; i++) {
        	...
        	double result = Nlll * L * Powerspectrum_Counterterm_Damping(kvec, los, alpha_perp, alpha_parallel, f, b1, c0, c1, c2, knl, Sigma2, dSigma2);
        	double jacobian = 1.0; //(2.0 / 2.0) * (2.0 * M_PI / (2.0 * M_PI));
